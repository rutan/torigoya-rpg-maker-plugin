...
${Array.from(state.useFunctions).map((n) => `import {${n}} from '../../../common/pickFromParameter/${n}'`).join('\n')}

export function readParameter() {
  const parameter = PluginManager.parameters(getPluginName());
  return {
    version: ${JSON.stringify(config.version)},
    ${Array.from(state.paramMap).map(([k, v]) => `${k}: ${v}`).join(',\n')}
  };
}
        `;
    ...
    if (!param.dummy) {
        ...
        if (funcName) {
            ...
            const code = `((parameter) => {
                ${paramName.includes('.') ? '' : `if (typeof parameter === 'string') parameter = JSON.parse(parameter);\n`}return {
                    ${Array.from(subState.paramMap).map(([k, v]) => `${k}: ${v}`).join(',\n')}
                };
            })(${paramName}.${param.key})`;
